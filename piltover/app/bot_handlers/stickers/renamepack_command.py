...
_text = "Choose the sticker set you want to rename."
_text_no_sets = "You have no sets :("  # TODO: correct text


class RenamePack(BotInteractionHandler[StickersBotState, StickersBotUserState]):
    def __init__(self) -> None:
        super().__init__(StickersBotUserState)
        self.command("renamepack").do(self._handler).register()
    ...
    async def _handler(peer: Peer, _message: MessageRef, _state: None) -> MessageRef:
        ...
        await StickersBotUserState.update_or_create(user=peer.owner, defaults={
            "state": StickersBotState.RENAMEPACK_WAIT_PACK,
            "data": None,
