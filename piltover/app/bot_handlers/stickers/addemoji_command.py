...
_text = "Choose a emoji set."
_text_no_sets, _text_no_sets_entities = FormatableTextWithEntities("""
You don't have any emoji sets yet. To create a set, use <c>/newemojipack</c>.
""".strip()).format()


class AddEmoji(BotInteractionHandler[StickersBotState, StickersBotUserState]):
    def __init__(self) -> None:
        super().__init__(StickersBotUserState)
        self.command("addemoji").do(self._handler).register()

    @staticmethod
    async def _handler(peer: Peer, _message: MessageRef, _state: None) -> MessageRef:
        keyboard_rows = await get_stickerset_selection_keyboard(peer.owner_id, True)
