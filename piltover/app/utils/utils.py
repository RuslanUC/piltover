...
async def resize_photo(
    ...
    for idx, (resized, width, height) in enumerate(res):
        ...
        if new_file_id is not None and new_as_document:
            await storage.save_big_part(new_file_id, 0, resized.getbuffer(), True)
            await storage.finalize_big_upload_as(new_file_id, StorageType.DOCUMENT, 0)

        await storage.save_big_part(save_file_id, 0, resized.getbuffer(), True, str(width))
        await storage.finalize_big_upload_as(save_file_id, StorageType.PHOTO, 0, str(width))
