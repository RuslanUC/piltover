...
class Chat(Model):
    ...
    async def get_private(user1: models.User, user2: models.User | None = None) -> Chat | None:
        return await Chat.get_or_none(
            type=ChatType.PRIVATE,
            id__in=Subquery(
                Chat.filter(dialogs__user__id__in=[user1.id, user2.id] if user2 is not None else [user1.id])
                .annotate(user_count=Count("dialogs__user__id", distinct=True))
                .filter(user_count=2 if user2 is not None else 1)
                .group_by("id")
    ...
    async def get_or_create_private(user1: models.User, user2: models.User | None = None) -> Chat:
        ...
        if chat is not None:
            chat = await Chat.create(type=ChatType.PRIVATE)
